 ...
 */
@Controller
@RequestMapping(value="series_movie")
public class SeriesMoviepageController {
    @RequestMapping(value = "/",method = RequestMethod.GET)
    public String genresId(ModelMap model){
        List<Film> films= new FilmService().getSeriesFilm();
        int row=(films.size()%6==0)?(films.size()/6):(films.size()/6)+1;
        model.addAttribute("row", row);
        model.addAttribute("films", films);
        model.addAttribute("see_many", new FilmService().getFilmSeeMany(true));
